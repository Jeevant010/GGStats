  ...
  useEffect(() => {
    const fetchGames = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://v1.basketball.api-sports.io/games?date=${dateStr}`, {
      ...
      <main className="flex-1 w-full text-white py-6 px-4 lg:px-6 max-w-[1400px] mx-auto">
        <div className="flex flex-col items-center gap-3 mb-6">
          <h2 className="text-2xl font-bold text-center tracking-wide">
            🏀 Basketball — {selectedDate.toDateString() === new Date().toDateString() ? "Today's" : selectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} Games
          </h2>
          ...
          <div className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-gray-400 text-lg">
                Loading games for {selectedDate ? selectedDate.toLocaleDateString() : "today"}...
              </div>
            </div>
          </div>
        )}
