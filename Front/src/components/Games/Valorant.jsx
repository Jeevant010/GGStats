...
const Valorant = () => {
    ...
    const groupedSchedule = groupByRegion(dateFilteredSchedule);
    const regions = ["All", ...Object.keys(groupedSchedule)];
    const matchesToShow = selectedRegion === "All" ? dateFilteredSchedule : (groupedSchedule[selectedRegion] || []);
                ...
                <main className="flex-1 w-full text-white py-6 px-4 lg:px-6 max-w-[1400px] mx-auto">
                    <div className="flex flex-col items-center gap-3 mb-6">
                        <h2 className="text-2xl font-bold text-center tracking-wide">
                            🎮 Valorant — {isToday ? "Today's" : selectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} Schedule
                        </h2>
                        ...
                        <>
                            ...
                            {matchesToShow.length === 0 && (
                                <div className="text-center text-gray-500 mt-10">
                                    No matches found for {isToday ? 'today' : selectedDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}.
                                </div>
                            )}
                        </>
                    )}
