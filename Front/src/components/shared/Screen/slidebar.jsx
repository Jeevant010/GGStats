...
export default function Slidebar({ isOpen }){
  ...
  return (
    <div>
        {/* Positioned on the right side now; slides in/out from the right */}
        <div className={`group main-slide absolute h-screen w-24 top-[80px] right-0 hover:w-[200px] transition-all duration-300 ease-in-out bg-black text-white shadow-lg z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            {/* <Logo /> */}
                        ...
                        <div className='menu-items relative flex flex-col gap-8 px-[6px] top-20 '>
                                {menuItems.map((item, index) => {
                                        const { Icon, label } = item; // keep explicit usage for linter
                                        return (
                    <div 
